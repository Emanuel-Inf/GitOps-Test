...
func TestPodDeploysContainerImage(t *testing.T) {
	...
	options := &helm.Options{
		SetValues: map[string]string{"image": "traefik:v2.10"},
	}
	...
	releaseName := fmt.Sprintf("traefik-%s", strings.ToLower(random.UniqueId()))
	...
	podName := fmt.Sprintf("%s-traefik-infrastruktur", releaseName)
...
func verifyNginxPod(t *testing.T, kubectlOptions *k8s.KubectlOptions, podName string) {
	...
	tunnel := k8s.NewTunnel(kubectlOptions, k8s.ResourceTypePod, podName, 8080, 8080)
	defer tunnel.Close()
